...
| trait / port | 当前 standalone 实现名 | 当前模块文件名 | 不一致说明 |
| --- | --- | --- | --- |
| `SessionLedgerPort` | `StandaloneSessionStore` | `crates/santi-db/src/adapter/standalone/session_store.rs` | trait 是 `Ledger`，实现仍用 `Store` |
| `SoulPort` | `StandaloneSoulStore` | `crates/santi-db/src/adapter/standalone/soul_store.rs` | trait 是 `Port`，实现仍用 `Store` |
...
| `SoulRuntimePort` | `StandaloneSoulRuntime` / `DbSoulRuntime` | `crates/santi-db/src/adapter/standalone/soul_runtime.rs` / `crates/santi-db/src/adapter/postgres/soul_runtime.rs` | 这组是目前最接近一致的一组 |
...
| 当前名字 | 建议目标名 |
| --- | --- |
| `StandaloneSessionStore` | `SessionLedgerPort` 对应实现名，优先收口到 `StandaloneSessionLedger` 或等价的 `Ledger` 命名 |
| `StandaloneSoulStore` | `SoulPort` 对应实现名，优先收口到 `StandaloneSoulPort` 或等价的 `Port` 命名 |
| `standalone/session_store.rs` | `standalone/session_ledger.rs` |
| `standalone/soul_store.rs` | `standalone/soul.rs` 或 `standalone/soul_port.rs` |
| `DbSessionLedger` | 维持或与 standalone 端统一到同一命名风格 |
