...
- **db adapters**：`crates/santi-db/src/adapter/{standalone,postgres}`

这条边界当前是后续落地的唯一参考线：port 负责抽象能力，adapter 负责具体存储实现。

在概念上，这里的主轴应固定为：`santi` 始终是同一个 HTTP 服务边界；差异是 **单机** 与 **分布式** 的依赖拓扑与 adapter family，而不是“是不是 service”。当前实现命名已经对齐为 `standalone` / `postgres` / `distributed`。

## 当前 trait -> impl 映射表

> 说明：这里只记录当前代码中已经存在的明确实现关系。

| trait / port | 单机侧现有 impl (`standalone`) | 分布式侧现有 impl (`postgres`) |
| --- | --- | --- |
...
- standalone impls：`crates/santi-db/src/adapter/standalone/*`
- postgres impls：`crates/santi-db/src/adapter/postgres/*`

## 当前不一致点

- **命名混杂**：`Store / Ledger / Runtime` 混用，例如 `StandaloneSessionStore` 实现的是 `SessionLedgerPort`，`StandaloneSoulStore` 实现的是 `SoulPort`。
- **单机 partial impl**：`StandaloneSessionStore::apply_message_event()` 仍返回 unavailable；单机的 compact / fork 能力已拆到各自的本地专用路径，而不是停留在一个复合 helper 里，说明 trait 边界已进一步收窄，但单机能力仍未完全对齐到同一组 ports。
- **单机 helper 已拆分**：当前没有 `StandaloneSessionForkCompactStore` 这类合并式 helper；fork 与 compact 分别由独立的 standalone 实现承载，仍是本地专用入口，而不是分布式 runtime ports 同构实现。
...
- 单机的 fork / compact 各自实现是否还需要继续收敛为更原子的 query seam
- compact / assembly 这类读取能力是否需要新的更原子的 query seam，而不是重新把 composite read 塞回 runtime port

## 最安全的后续执行顺序

1. **先确认 canonical path**：把 `adapter/standalone/*` 与 `adapter/postgres/*` 作为唯一主入口的事实完全收口。
...
- canonical path 已经收口到 `adapter/standalone/*` 与 `adapter/postgres/*`；root-level 旧实现文件和遗留 helper 已不再是事实入口。
- 目标转为：在不放宽 `SoulRuntimePort` 的前提下，补齐仍真正影响主路径的能力，并决定 compact/list 这类读取要落在哪个更小的边界上；也就是继续收紧单机 / 分布式拓扑下的 adapter 边界，而不是重谈 service 边界。
...
- standalone:
  ...
  - `crates/santi-db/src/adapter/standalone/soul_runtime.rs`
  - `crates/santi-db/src/adapter/standalone/session_fork.rs`
  - `crates/santi-db/src/adapter/standalone/session_compact.rs`
- postgres:
...
`adapter/standalone/*` 与 `adapter/postgres/*` 现在已经是唯一事实入口；不再通过 root-level 实现文件承载真实逻辑。
...
- 暴露 `adapter::standalone` 与 `adapter::postgres`
...
1. **保持 consumer 入口只走 canonical path**
   - 所有调用方继续只从 `adapter/standalone/*` 与 `adapter/postgres/*` 进入。
2. **继续收 contract seam**
   - 把仍会影响主路径的 unsupported / partial impl 收到最小必要集合。
3. **把读取能力缺口放到单独 seam 处理**
   - 尤其是 compact/list 这类分布式读取路径，不要回退成大而全的 composite runtime port。
...
- 不要把 standalone 的 partial impl 当作已经完成的对等实现。
